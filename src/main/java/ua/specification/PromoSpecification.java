...
public class PromoSpecification implements Specification<Book> {
private final UserFilter filter;
	...
	public PromoSpecification(UserFilter filter) {
		super();
		this.filter = filter;
	}
	
	public void findAllPromo(Root<Book> root, CriteriaQuery<?> query, CriteriaBuilder cb){
		Join<Book, BookPromo> bt=root.join("bookpromo");
		predicates.add(cb.like(bt.get("promo"),filter.getFindTopic()));
	}
